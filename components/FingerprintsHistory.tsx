...
import {
  StyleSheet, View, Text, ScrollView, Pressable, Alert, Image,
} from 'react-native';
import { SafeAreaView } from 'react-native-safe-area-context';
import { useFocusEffect } from '@react-navigation/native';
import { listSensorRecords, SensorRecord, deleteSensorRecords } from '../services/db';
...
function recordToLegacy(record: SensorRecord): SavedFingerprintData {
  return {
    ...
    deltaReadings: (record.deltaCh4 !== null) ? {
      CH4: record.deltaCh4 ?? 0, NH3: record.deltaNh3 ?? 0, HCHO: record.deltaHcho ?? 0,
      VOC: record.deltaVoc ?? 0, Odour: record.deltaOdour ?? 0, H2S: record.deltaH2s ?? 0,
      Etoh: record.deltaEtoh ?? 0, NO2: record.deltaNo2 ?? 0,
    } : undefined,
  };
}
...
export default function FingerprintsHistory() {
  ...
  useFocusEffect(useCallback(() => { loadFingerprints(); }, [loadFingerprints]));

  const toggleSelectMode = () => { setIsSelectMode(p => !p); setSelectedIds([]); };

  const handleDelete = () => {
    Alert.alert(
      'Delete fingerprints',
      `Delete ${selectedIds.length} fingerprint${selectedIds.length !== 1 ? 's' : ''}? This cannot be undone.`,
      [
  ...
  if (showComparison) {
    ...
    return <ComparisonView selectedItems={selectedLegacy} onBack={() => setShowComparison(false)} />;
  }

  if (expandedIndex !== null && items[expandedIndex]) {
    const rec = items[expandedIndex];
    const tags = rec.tagsJson ? (() => { try { return JSON.parse(rec.tagsJson!); } catch { return []; } })() : [];
    return (
      <ExpandedFingerprintView
        data={recordToLegacy(rec)}
        sensorRecordId={rec.id}
        initialTags={tags}
        onBack={() => { setExpandedIndex(null); loadFingerprints(); }}
      />
    );
  }
    ...
    <SafeAreaView style={styles.container}>
        ...
        <View style={styles.headerButtons}>
          {!isSelectMode ? (
            ...
            <>
              ...
              <Pressable
                style={[styles.analyseButton, selectedIds.length < 2 && styles.disabledButton]}
      ...
      <ScrollView style={styles.listSection} contentContainerStyle={styles.listContent}>
        {items.map((record, idx) => {
          ...
          const tags: string[] = record.tagsJson ? (() => { try { return JSON.parse(record.tagsJson!); } catch { return []; } })() : [];
          return (
            <Pressable
              key={record.id}
              onPress={() => handleFingerprintPress(idx, record.id)}
              style={[styles.fingerprintCard, isSelectMode && isSelected && styles.selectedCard]}
            >
              <View style={styles.cardContent}>
                {isSelectMode && (
                  <View style={styles.selectionCircle}>
                    {isSelected ? (
                      <View style={styles.selectedCircle}><View style={styles.selectedCircleInner} /></View>
                    ) : (
                ...
                <View style={styles.textContent}>
                  <Text style={styles.cardTitle}>{record.title || 'Untitled'}</Text>
                  <Text style={styles.cardDescription}>{record.description || 'No description'}</Text>
                  {tags.length > 0 && (
...
const styles = StyleSheet.create({
  container: { flex: 1 },
  header: { flexDirection: 'row', justifyContent: 'flex-end', alignItems: 'center', paddingHorizontal: MARGIN, paddingVertical: 20 },
  headerButtons: { flexDirection: 'row', gap: 10 },
  selectButton: { paddingHorizontal: 24, paddingVertical: 10, borderRadius: 20, borderWidth: 1, borderColor: '#000', backgroundColor: '#fff' },
  selectButtonText: { fontSize: 16, color: '#000', fontWeight: '500' },
  cancelButton: { paddingHorizontal: 24, paddingVertical: 10, borderRadius: 20, borderWidth: 1, borderColor: '#000', backgroundColor: '#fff' },
  cancelButtonText: { fontSize: 16, color: '#000', fontWeight: '500' },
  analyseButton: { paddingHorizontal: 24, paddingVertical: 10, borderRadius: 20, backgroundColor: '#000' },
  analyseButtonText: { fontSize: 16, color: '#fff', fontWeight: '500' },
  deleteButton: { paddingHorizontal: 24, paddingVertical: 10, borderRadius: 20, backgroundColor: '#ff3b30' },
  deleteButtonText: { fontSize: 16, color: '#fff', fontWeight: '500' },
  disabledButton: { opacity: 0.4 },
  listSection: { flex: 1 },
  listContent: { paddingHorizontal: MARGIN, paddingBottom: 100 },
  fingerprintCard: { marginBottom: GUTTER, borderRadius: 12, borderWidth: 1, borderColor: '#e0e0e0', backgroundColor: '#fff', overflow: 'hidden' },
  ...
  unselectedCircle: { width: 24, height: 24, borderRadius: 12, borderWidth: 2, borderColor: '#666' },
  selectedCircle: { width: 24, height: 24, borderRadius: 12, backgroundColor: '#000', justifyContent: 'center', alignItems: 'center' },
  selectedCircleInner: { width: 10, height: 10, borderRadius: 5, backgroundColor: '#fff' },
  textContent: { flex: 1, marginRight: 12 },
  cardTitle: { fontSize: 16, fontWeight: '600', color: '#000', marginBottom: 6 },
