...
import { insertSensorRecord, insertCapture } from '../services/db';
...
const MIC_LABEL: Record<MicPhase, string> = {
  idle: '', recording: 'Recording…', uploading: 'Uploading…',
  processing: 'Transcribing…', error: 'Failed — tap to retry',
};
...
export default function FingerprintModal({ visible, onClose }: FingerprintModalProps) {
      ...
      pollRef.current = setInterval(async () => {
        try {
          ...
          if (status?.status === 'transcribed') {
            ...
            if (transcript) { setDescription(transcriptToPlainText(transcript)); }
            setMicPhase('idle');
            setRecordingDuration('00:00');
        ...
        } catch (e) { console.warn('STT poll error (will retry):', e); }
      }, 10_000);
    } catch (err) {
  ...
  const handleSave = async () => {
    ...
    const r = (fingerprint.olfactoryData?.readings ?? {}) as Record<string, number>;
    ...
    try {
      await insertSensorRecord({
        ...
        deltaCh4: null, deltaNh3: null, deltaHcho: null, deltaVoc: null,
        deltaOdour: null, deltaH2s: null, deltaEtoh: null, deltaNo2: null,
      });
      ...
      await enqueueSync({
        ...
        payloadJson: JSON.stringify({
          ...
          location: location ?? null, readings: r,
        }),
        createdAt: Date.now(),
      });
  ...
  const handleCancel = async () => {
    ...
    setTitle(''); setDescription(''); setPhotoPath(null);
    setMicPhase('idle'); setRecordingDuration('00:00');
    audioPathRef.current = null; recordingIdRef.current = null;
  ...
  React.useEffect(() => {
    if (visible && !capturedFingerprint) { setCapturedFingerprint(createFingerprint()); }
  }, [visible]);
  ...
  return (
    <Modal visible={visible} animationType="slide" transparent={false} onRequestClose={handleCancel}>
      {showCamera ? (
        <CameraModal onClose={() => setShowCamera(false)} onPhotoTaken={handlePhotoTaken} />
      ) : (
        ...
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.container}>
          <ScrollView contentContainerStyle={styles.scrollContent}>
            ...
            <View style={styles.inputCard}>
              <TextInput
                style={styles.titleInput} placeholder="Title" placeholderTextColor="#999"
              ...
              <View style={styles.noteBar}>
                <TextInput
                  ...
                  multiline numberOfLines={3} textAlignVertical="top" editable={!isBusy}
                />
                <TouchableOpacity
                  ...
                  onPress={handleMicPress} disabled={isBusy}
                >
                  {micPhase === 'recording' ? (
              ...
              {micPhase !== 'idle' && (
                <View style={styles.statusRow}>
                  {micPhase === 'recording' && <View style={styles.recordingDot} />}
                  <Text style={[styles.statusText, micPhase === 'error' && styles.statusTextError]}>
                    {micPhase === 'recording' ? recordingDuration : statusLabel}
                  </Text>
                </View>
              )}
            </View>
            ...
            {photoPath && (
              <View style={styles.photoPreviewContainer}>
                <Image source={{ uri: `file://${photoPath}` }} style={styles.photoPreview} resizeMode="cover" />
                <TouchableOpacity style={styles.removePhotoButton} onPress={() => setPhotoPath(null)}>
...
const styles = StyleSheet.create({
  ...
  saveText: { fontSize: 16, color: '#007AFF', fontWeight: '600', textAlign: 'right' },
  inputCard: {
    backgroundColor: '#fff', marginHorizontal: 16, marginTop: 20, borderRadius: 12, padding: 16,
    shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.1, shadowRadius: 3, elevation: 2,
  },
  titleInput: { fontSize: 16, fontWeight: '600', color: '#000', paddingVertical: 8 },
  divider: { height: 1, backgroundColor: '#e0e0e0', marginVertical: 8 },
  noteBar: { flexDirection: 'row', alignItems: 'flex-end', gap: 8, paddingTop: 4 },
  noteInput: { flex: 1, fontSize: 15, color: '#000', paddingVertical: 8, minHeight: 60, maxHeight: 120 },
  micButton: {
  ...
  photoIconContainer: { width: 32, height: 32, borderRadius: 6, backgroundColor: '#f0f0f0', justifyContent: 'center', alignItems: 'center' },
  photoIcon: { fontSize: 18 },
  photoPreviewContainer: { marginHorizontal: 16, marginTop: 20, borderRadius: 12, overflow: 'hidden', position: 'relative' },
  ...
  chartCard: {
    backgroundColor: '#fff', marginHorizontal: 16, marginTop: 20, marginBottom: 8,
