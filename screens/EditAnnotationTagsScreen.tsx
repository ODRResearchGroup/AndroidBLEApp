...
import { getCapture, updateCaptureTags, updateCaptureDescription } from '../services/db';
...
export default function EditAnnotationTagsScreen({ navigation, route }: Props) {
    ...
    (async () => {
      try {
        ...
        if (ann) {
          ...
          const row = await getCapture(annotationId);
          const suggested: string[] = row?.suggestedTagsJson ? JSON.parse(row.suggestedTagsJson) : [];
          setSuggestedTags(suggested.filter(t => !(ann!.selectedTags ?? []).includes(t)));
          if (row) { setCaptureStatus(row.status); }
        }
      } catch (e) { console.warn('Failed to load annotation:', e); }
  ...
  const handleSave = async () => {
    if (!annotation || saving) { return; }
    const transcriptionPending = annotation.type === 'audio'
      && captureStatus !== null
  ...
  if (loading) {
    return (
      <SafeAreaView style={styles.safe}>
        <View style={styles.loadingContainer}><ActivityIndicator size="large" color={C.darkGray} /></View>
      </SafeAreaView>
    );
  }
  ...
  if (!annotation) {
    return (
      <SafeAreaView style={styles.safe}>
        <View style={styles.loadingContainer}><Text style={styles.errorText}>Annotation not found.</Text></View>
      </SafeAreaView>
    );
  }
...
const styles = StyleSheet.create({
  ...
  descriptionInput: { fontSize: 14, color: C.black, lineHeight: 20, minHeight: 60, marginBottom: 12, borderWidth: 1, borderColor: C.border, borderRadius: 8, padding: 8 },
