...
import {
  View, Text, TextInput, ScrollView, Animated, TouchableWithoutFeedback,
  Image, StyleSheet, SafeAreaView, StatusBar, ActivityIndicator, RefreshControl,
} from 'react-native';
import { NativeStackScreenProps } from '@react-navigation/native-stack';
import { useFocusEffect } from '@react-navigation/native';
import { RootStackParamList } from '../App';
import { listCaptures, updateCaptureTags, updateCaptureDescription, CaptureRow } from '../services/db';
import { saveAnnotation } from '../services/annotationService';
import { deduplicateTranscript } from '../services/processedTranscript';
...
function rowToCard(row: CaptureRow, i: number): CardData {
  return {
    ...
    type:           row.type,
    timestamp:      row.latitudeDisplay ? new Date(row.capturedAt).toLocaleTimeString() : '00:00:00',
    latitude:       row.latitudeDisplay  ?? '00.0000° N',
    longitude:      row.longitudeDisplay ?? '00.0000° E',
    description:    deduplicateTranscript(row.description || (row.transcriptJson
...
export default function AnnotationFeed({ navigation }: Props) {
  ...
  const persistTagChange = async (captureId: string, selected: string[], suggested: string[]) => {
    try {
  ...
  const handleDescriptionChange = async (captureId: string, text: string) => {
    setCards(prev => prev.map(c => c.captureId === captureId ? { ...c, description: text } : c));
    try {
    ...
    } catch (e) { console.warn('Description save failed (non-fatal):', e); }
  };
      ...
      >
        {cards.length === 0 ? (
          <View style={styles.emptyState}>
            ...
            <Text style={styles.emptyBody}>Tap "Add New Annotation" to record a note linked to a smell walk entry.</Text>
          </View>
        ) : (
          cards.map(card => (
            <View key={card.captureId} style={styles.card}>
              ...
              <View style={styles.annotationRow}>
                ...
                <TouchableWithoutFeedback
                  onPress={() => navigation.navigate('EditAnnotationTags', { annotationId: card.captureId })}>
              ...
              {card.type === 'photo' ? (
                <>
                  <TextInput
                    ...
                    onChangeText={text => setCards(prev => prev.map(c => c.captureId === card.captureId ? { ...c, description: text } : c))}
                    onBlur={() => handleDescriptionChange(card.captureId, card.description)}
              ...
              <View style={styles.selectedTagsWrap}>
                {card.selectedTags.map(t => (
                  <TagPill key={t} label={t} mode="selected" onPress={() => handleRemoveTag(card.captureId, t)} />
                ))}
              </View>
              {card.suggestedTags.length > 0 && (
                <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.suggestedTagsRow}>
                  {card.suggestedTags.map(t => (
                    <TagPill key={t} label={t} mode="suggested" onPress={() => handleAddTag(card.captureId, t)} />
                  ))}
                </ScrollView>
              )}
            </View>
...
const styles = StyleSheet.create({
  ...
  emptyTitle: { fontSize: 18, fontWeight: '600', color: C.black, marginBottom: 10 },
  emptyBody: { fontSize: 14, color: C.lightGray, textAlign: 'center', lineHeight: 20 },
  card: {
    backgroundColor: C.white, borderRadius: 12, borderWidth: 1, borderColor: C.border,
    marginBottom: 20, paddingHorizontal: 16, paddingTop: 14, paddingBottom: 16,
  ...
  annotationLabel: { fontSize: 11, fontWeight: '600', color: C.darkGray, letterSpacing: 1.2 },
  editIcon: { fontSize: 20, color: C.darkGray, lineHeight: 24 },
  description: { fontSize: 14, color: C.black, lineHeight: 20, marginBottom: 12, textAlign: 'justify' },
  ...
  transcript: { fontSize: 14, color: C.black, lineHeight: 22, marginBottom: 14, minHeight: 44, borderWidth: 1, borderColor: C.border, borderRadius: 8, paddingHorizontal: 10, paddingVertical: 8 },
  tagsLabel: { fontSize: 11, fontWeight: '600', color: C.darkGray, letterSpacing: 1.2, marginBottom: 8 },
  ...
  fabContainer: {
    position: 'absolute', bottom: 0, left: 0, right: 0, paddingHorizontal: 24, paddingBottom: 32,
    paddingTop: 12, backgroundColor: C.white,
  ...
  fabLabel: { fontSize: 16, fontWeight: '600', color: C.white, letterSpacing: 0.3 },
});
