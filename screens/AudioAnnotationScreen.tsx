  ...
  return (
    <Animated.View style={[styles.waveform, isActive && { transform: [{ scaleY: pulse }] }]}>
      {WAVEFORM_BARS.map((h, i) => (
        <View key={i} style={[styles.waveBar, { height: 4 + h * 28, backgroundColor: isActive ? C.black : C.lightGray }]} />
      ))}
    </Animated.View>
  );
};
...
export default function AudioAnnotationScreen({ navigation, route }: Props) {
  ...
  const handleStart = async () => {
    try {
      ...
      await Promise.all([startAudioRecording(recordingId), startAudioTrack(startedAtMsRef.current)]);
      setElapsed(0);
      setPhase('recording');
      startTimer();
    } catch (err: any) {
  ...
  const handleStop = async () => {
    ...
    try {
      ...
      Alert.alert('Recording error', err.message ?? String(err));
      setPhase('error'); setStatusLabel(''); return;
    }
    ...
    const baseCapture = {
      id: recordingId, sensorRecordId: sensorRecordId ?? null, type: 'audio' as const,
      localPath: stopResult.localUri, imageBlobName: null, imageContainer: null,
      transcriptJson: null, selectedTagsJson: null, suggestedTagsJson: null, description: '',
      latitudeDisplay: coords.latitude, longitudeDisplay: coords.longitude,
      latitudeRaw: firstPt?.lat ?? null, longitudeRaw: firstPt?.lon ?? null,
      capturedAt: startedAt, annotationIndex,
    };
    ...
    try {
      ...
      await Promise.all([
        uploadAudioToAzure(stopResult.localUri, prep.audioUpload.uploadUrl, prep.audioUpload.container, prep.audioUpload.blobName),
        uploadAudioTrackJsonToAzure(trackBundle, prep.bundleUpload.uploadUrl, prep.bundleUpload.container, prep.bundleUpload.blobName,
          { container: prep.audioUpload.container, blobName: prep.audioUpload.blobName }),
      ]);
      await insertCapture({ ...baseCapture, bundleBlobName: prep.bundleUpload.blobName, status: 'uploaded' });
      await notifyUploadComplete(recordingId, 'm4a');
      uploadedOk = true;
    } catch {
      await insertCapture({ ...baseCapture, bundleBlobName: null, status: 'pending' }).catch(() => {});
      await enqueueSync({
        id: uuidv4(), entityType: 'capture', entityId: recordingId,
        operation: 'create', createdAt: Date.now(),
        payloadJson: JSON.stringify({
          type: 'audio', captureId: recordingId, localPath: stopResult.localUri,
          audioExt: 'm4a', locale: 'en-US', trackBundle,
          sensorRecordId: sensorRecordId ?? null, capturedAt: startedAt, annotationIndex,
        }),
      }).catch(() => {});
    }

    setPhase('done');
    setStatusLabel(uploadedOk ? 'Processing in background…' : 'Saved offline — will sync when online');
  ...
  const handleSaveTyped = async () => {
    ...
    if (!text) { Alert.alert('Empty note', 'Write something before saving.'); return; }
    setSavingText(true);
    ...
    } finally { setSavingText(false); setStatusLabel(''); }
  };
        ...
        <View style={styles.card}>
            ...
            <View style={styles.modeRow}>
              {(['record', 'type'] as InputMode[]).map(m => (
                <TouchableWithoutFeedback
                  key={m}
                  onPress={() => { modeAnim.fireHaptic(); setInputMode(m); }}
                  {...modeAnim.handlers}>
                  <Animated.View style={[
                    styles.modeBtn,
                    ...
                    {m === 'record'
                      ? <Mic  size={16} color={inputMode === m ? C.white : C.lightGray} strokeWidth={2} />
                      : <Type size={16} color={inputMode === m ? C.white : C.lightGray} strokeWidth={2} />
                    }
                    <Text style={[styles.modeBtnLabel, inputMode === m && styles.modeBtnLabelActive]}>
            ...
            <View style={styles.typeContainer}>
              ...
              <TouchableWithoutFeedback
                ...
                <Animated.View style={[
                  ...
                  <Text style={styles.actionButtonLabel}>{savingText ? 'Saving…' : 'Save Note'}</Text>
                </Animated.View>
              </TouchableWithoutFeedback>
            </View>
          )}
          ...
          {inputMode === 'record' && (
            <>
              ...
              {statusLabel !== '' && (
                <Text style={[styles.statusText, phase === 'error' && { color: C.error }]}>
                  {statusLabel}
                </Text>
              )}
...
const styles = StyleSheet.create({
  ...
  card:                 { flex: 1, borderRadius: 16, borderWidth: 1, borderColor: C.border, backgroundColor: C.white, paddingHorizontal: 24, paddingTop: 20, paddingBottom: 28, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.05, shadowRadius: 4, elevation: 2 },
  annotationLabel:      { fontSize: 11, fontWeight: '600', color: C.darkGray, letterSpacing: 1.2, marginBottom: 12 },
  headerDivider:        { height: 1, backgroundColor: C.border, marginBottom: 20 },
  modeRow:              { flexDirection: 'row', gap: 10, marginBottom: 20 },
  modeBtn:              { flexDirection: 'row', alignItems: 'center', gap: 6, paddingHorizontal: 16, paddingVertical: 9, borderRadius: 10, borderWidth: 1.5, borderColor: C.border, backgroundColor: C.white },
  ...
  typeInput:            { fontSize: 15, color: C.black, lineHeight: 22, minHeight: 140, borderWidth: 1, borderColor: C.border, borderRadius: 10, padding: 12 },
  title:                { fontSize: 22, fontWeight: '700', color: C.black, textAlign: 'center' },
  spacer:               { flex: 1, minHeight: 20 },
  duration:             { fontSize: 18, color: C.darkGray, textAlign: 'center', letterSpacing: 1, marginBottom: 14 },
  ...
  actionButtonLabel:    { fontSize: 16, fontWeight: '600', color: C.white, letterSpacing: 0.3 },
});
