...
import { Canvas, Path, Skia, useCanvasRef, useImage, Image as SkiaImage } from '@shopify/react-native-skia';
...
type Stroke = { id: string; points: Point[]; color: string; strokeWidth: number };
...
function closestStrokeId(strokes: Stroke[], x: number, y: number, threshold = 18): string | null {
...
const Slider = ({ value, onChange, track }: { value: number; onChange: (v: number) => void; track: React.ReactNode }) => {
  ...
  const pr = useRef(PanResponder.create({
    ...
    onPanResponderGrant: (e: GestureResponderEvent) =>
      onChange(Math.min(1, Math.max(0, e.nativeEvent.locationX / (tw.current || 1)))),
...
const WidthTrack = () => (
  <View style={styles.widthTrack}>
    {Array.from({ length: 20 }).map((_, i) => (
      <View key={i} style={{ flex: 1, height: 1 + i * 0.5, backgroundColor: C.darkGray, borderRadius: 1, alignSelf: 'center' }} />
    ))}
  </View>
);
...
const DrawingCanvas = ({
  strokes, onAddStroke, onErase, currentColor, strokeWidth, isErasing,
  photoUri, canvasRef,
}: {
  strokes: Stroke[];
    ...
    >
      <Canvas ref={canvasRef} style={StyleSheet.absoluteFill}>
        {photoImage && canvasSize.w > 0 && (
          <SkiaImage image={photoImage} x={0} y={0} width={canvasSize.w} height={canvasSize.h} fit="cover" />
        )}
        {strokes.map(s => <SkiaStroke key={s.id} stroke={s} />)}
        {livePoints.length > 1 && (
          <Path
            path={livePath}
            color={currentColor}
...
export default function PhotoAnnotationScreen({ navigation, route }: Props) {
  const {
    annotationIndex, photoUri, timestamp, capturedAtMs,
    latitude, longitude, latitudeRaw, longitudeRaw, accuracyM, sensorRecordId,
  } = route.params;
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (snapshot) {
        ...
        const tempPath = RNFS.TemporaryDirectoryPath.replace(/\/$/, '') + `/${captureId}_annotated.png`;
        await RNFS.writeFile(tempPath, base64, 'base64');
        uploadUri = 'file://' + tempPath;
      }

      const gpsPoint = (latitudeRaw !== null && longitudeRaw !== null)
        ? { t_ms: 0, lat: latitudeRaw, lon: longitudeRaw, accuracy_m: accuracyM ?? undefined }
        : null;

      const serialisedStrokes = strokes.map(s => ({
        id: s.id, points: s.points, color: s.color,
        shape: (s.strokeWidth - 1.5) / 8.5,
      }));

      const baseCapture = {
        id: captureId, sensorRecordId: sensorRecordId ?? null, type: 'photo' as const,
        localPath: uploadUri, transcriptJson: null, selectedTagsJson: null,
        suggestedTagsJson: null, description, latitudeDisplay: latitude,
        longitudeDisplay: longitude, latitudeRaw: latitudeRaw ?? null,
        longitudeRaw: longitudeRaw ?? null, capturedAt: capturedAtMs, annotationIndex,
      };
      ...
      try {
        ...
        const bundle: PhotoBundle = {
          schema: 'photo_gps_bundle_v1', captured_at_ms: capturedAtMs,
          photo: { container: prep.imageUpload.container, blobName: prep.imageUpload.blobName, contentType: 'image/png' },
          point: gpsPoint,
        };

        setSaveStatus('Uploading photo…');
        await Promise.all([
          uploadPhotoToAzure(uploadUri, prep.imageUpload.uploadUrl, prep.imageUpload.container, prep.imageUpload.blobName),
          uploadPhotoBundleJsonToAzure(bundle, prep.bundleUpload.uploadUrl, prep.bundleUpload.container, prep.bundleUpload.blobName),
        ]);

        setSaveStatus('Verifying upload…');
        await notifyPhotoUploadComplete(captureId, prep.imageUpload.blobName, prep.bundleUpload.blobName);
        await insertCapture({ ...baseCapture, bundleBlobName: prep.bundleUpload.blobName, imageBlobName: prep.imageUpload.blobName, imageContainer: prep.imageUpload.container, status: 'uploaded' });

        setSaveStatus('Saving annotation…');
        await saveAnnotation({
          recordingId: captureId, sensorRecordId: sensorRecordId ?? undefined,
          type: 'photo', description, strokes: serialisedStrokes, selectedTags: [],
          timestamp, latitude, longitude,
          latitudeRaw: latitudeRaw ?? undefined, longitudeRaw: longitudeRaw ?? undefined,
          accuracyM: accuracyM ?? undefined, capturedAtMs, photoUri: uploadUri, annotationIndex,
          imageBlobName: prep.imageUpload.blobName, imageContainer: prep.imageUpload.container,
          bundleBlobName: prep.bundleUpload.blobName, bundleContainer: prep.bundleUpload.container,
        });
        uploadedOk = true;
      } catch {
        ...
        try {
          const permanentPath = RNFS.DocumentDirectoryPath.replace(/\/$/, '') + `/pending_${captureId}.png`;
          await RNFS.copyFile(uploadUri.replace(/^file:\/\//, ''), permanentPath);
          permanentUri = 'file://' + permanentPath;
        } catch {}

        await insertCapture({ ...baseCapture, localPath: permanentUri, bundleBlobName: null, imageBlobName: null, imageContainer: null, status: 'pending' }).catch(() => {});
        await enqueueSync({
          id: uuidv4(), entityType: 'capture', entityId: captureId,
          operation: 'create', createdAt: Date.now(),
          payloadJson: JSON.stringify({
            type: 'photo', captureId, compositedUri: permanentUri, gpsPoint,
            description, strokesJson: JSON.stringify(serialisedStrokes), selectedTags: [],
            timestamp, latitude, longitude,
            latitudeRaw: latitudeRaw ?? null, longitudeRaw: longitudeRaw ?? null,
            accuracyM: accuracyM ?? null, capturedAtMs,
            sensorRecordId: sensorRecordId ?? null, annotationIndex,
          }),
        }).catch(() => {});
      }
        ...
        <View style={styles.card}>
          ...
          <View style={styles.toolbar}>
            ...
            <View style={styles.strokePreview}>
              <View style={[styles.strokePreviewLine, {
                height: strokeWidth,
          ...
          <TouchableWithoutFeedback onPress={handleSave} {...saveAnim.handlers} disabled={saving}>
            <Animated.View style={[
              ...
              <Text style={styles.saveButtonLabel}>{saving ? 'Saving…' : 'Save Annotation'}</Text>
            </Animated.View>
          </TouchableWithoutFeedback>
        </View>
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  annotationLabel:  { fontSize: 11, fontWeight: '600', color: C.darkGray, letterSpacing: 1.2, marginBottom: 6 },
  descriptionInput: { fontSize: 14, color: C.black, lineHeight: 20, minHeight: 48, marginBottom: 12 },
  ...
  strokePreview:    { flex: 1, alignItems: 'center', justifyContent: 'center', paddingHorizontal: 16 },
  strokePreviewLine: { width: '80%' },
  colorDot:         { width: 28, height: 28, borderRadius: 14, borderWidth: 1.5, borderColor: C.border },
  photoWrapper:     { height: 280, borderRadius: 10, overflow: 'hidden', backgroundColor: '#111', marginBottom: 20 },
  ...
  sliderLabel:      { fontSize: 12, fontWeight: '600', color: C.black, letterSpacing: 0.3, marginBottom: 8 },
  ...
  widthTrack:       { height: 12, borderRadius: 6, backgroundColor: '#eee', flexDirection: 'row', alignItems: 'center', paddingHorizontal: 4, gap: 3, overflow: 'hidden' },
  ...
  statusLabel:      { fontSize: 13, color: C.darkGray, textAlign: 'center', marginBottom: 12 },
  ...
  saveButtonLabel:  { fontSize: 16, fontWeight: '600', color: C.white, letterSpacing: 0.3 },
});
