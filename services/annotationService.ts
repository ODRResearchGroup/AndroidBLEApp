...
export type SaveAnnotationPayload = {
  ...
  selectedTags: string[];
  timestamp: string;
  latitude: string;            // display string e.g. "55.6050° N"
  longitude: string;
  latitudeRaw?: number;        // raw float for GPS/mapping
  longitudeRaw?: number;
  accuracyM?: number;
  capturedAtMs?: number;
  photoUri?: string;
  annotationIndex: number;
  // Azure Blob references — stored so get-photo-url can be called later
  imageBlobName?: string;      // e.g. "photos/abc-123.jpg"
  imageContainer?: string;     // e.g. "photos"
...
/**
 * Persists an annotation to Azure Blob Storage.
 * If an annotation with the same recordingId already exists it will be overwritten
 * (the backend uses upload with no if-none-match condition).
 */
export async function saveAnnotation(
  ...
  return { annotationId: json.annotationId };
}
