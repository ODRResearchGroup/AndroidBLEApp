...
/**
 * Calls photo-upload-complete on the backend after both the image and bundle
 * have been PUT to Azure Blob Storage. The backend verifies both blobs exist
 * and writes a status record so the capture is tracked like audio recordings.
 */
export async function notifyPhotoUploadComplete(
  ...
  if (!baseUrl || !functionKey) {
    throw new Error('Missing AZURE_FUNCTION_BASE_URL or AZURE_PHOTO_UPLOAD_COMPLETE_KEY');
  }
