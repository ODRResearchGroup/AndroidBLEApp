...
try { HapticFeedback = require('react-native-haptic-feedback').default; } catch { /* no-op */ }
...
export function usePressAnimation(options: Options = {}) {
  ...
  const fireHaptic = () => {
    if (haptic === 'none' || !HapticFeedback) { return; }
  ...
  return { scale, handlers: { onPressIn: pressIn, onPressOut: pressOut }, fireHaptic };
}
