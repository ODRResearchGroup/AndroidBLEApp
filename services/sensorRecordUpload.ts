...
function getConfig() {
  ...
  if (!baseUrl || !functionKey) {
    throw new Error('Missing AZURE_FUNCTION_BASE_URL or AZURE_SAVE_SENSOR_RECORD_KEY');
  }
...
export async function uploadSensorRecord(record: SensorRecord): Promise<void> {
  ...
  const url = `${baseUrl}/api/save-sensor-record?code=${encodeURIComponent(functionKey)}`;
